...
class ScAtdArgs:
    def __init__(self, drug_name):
        ...
        self.pretrained_model_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "frameworks", "scATD", "pretrained_models", "checkpoint_fold1_epoch_30.pth")
        )
        self.tune_threshold = True
        self.binarize_source = True
...
WANDB_PROJECT = "hyper_tuning_v5_balanced_minority"
MIN_MINOR_FRACTION = 0.25
...
def _compute_binarization_threshold(
    y_source: pd.Series, min_minor_fraction: float = MIN_MINOR_FRACTION, base_threshold: float = 0.5
) -> float:
    """Shift threshold toward minority until it reaches min_minor_fraction."""
...
def prepare_data(drug, target_tag, all_files, data_dir):
    """Loads, preprocesses, and splits the data for a given drug and target using balanced-minority binarization."""
    ...
    # Determine balanced-minority binarization threshold from source labels
    bin_threshold = _compute_binarization_threshold(y_source)
    y_source_bin = _binarize_series(y_source, bin_threshold)
    ...
    # for stratification
    y_source_train_bin = _binarize_series(y_source_train, bin_threshold)
    ...
    # target: train 80%, test 20%, stratified by balanced-minority bins when possible
    y_target_bin = _binarize_series(y_target_series, bin_threshold)
    ...
    X_target_train, X_target_test, y_target_train, y_target_test = train_test_split(
        X_target, y_target_series, test_size=0.2, random_state=SEED, stratify=stratify_target
    )

    # DEBUG statements
    # label distribution
    print(
        f"Source train label counts {np.bincount(_binarize_series(y_source_train, bin_threshold))}, "
        f"val counts {np.bincount(_binarize_series(y_source_val, bin_threshold))}, "
        f"test counts {np.bincount(_binarize_series(y_source_test, bin_threshold))}"
    )
    print(
        f"Target train label counts {np.bincount(_binarize_series(y_target_train, bin_threshold))}, "
        f"test counts {np.bincount(_binarize_series(y_target_test, bin_threshold))}"
    )
    print(f"Using binarization threshold {bin_threshold:.4f} (balanced-minority)")
    ...
    return {
        ...
        "y_target_independent": None,  # independent target not needed in this hyperparameter tuning script
    ...
    for drug in cli_args.drugs:
        ...
        for target_tag in target_file_names.get(drug, []):
            ...
            study = optuna.create_study(
                ...
                study_name=f"{drug}_{target_tag}_{model_name}",
            )
