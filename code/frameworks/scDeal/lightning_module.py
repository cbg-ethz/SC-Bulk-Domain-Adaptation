...
class scDEALLightningModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if torch.unique(self._harden_labels(y_src)).size(0) < 2:
            return None
        ...
        loss_c = self._classification_loss(y_pre, y_src)
        ...
        y_true = self._harden_labels(y_src).cpu().numpy()
    ...
    def _classification_loss(self, logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        labels = self._soft_labels(labels).to(logits.device)
        pos_logit = self._positive_logit(logits)
        loss = F.binary_cross_entropy_with_logits(pos_logit, labels)
        return loss
...
class PredictorLightningModule(_BestWeightMixin, pl.LightningModule):
    ...
    def _positive_logit(logits: torch.Tensor) -> torch.Tensor:
        ...
        if logits.size(-1) >= 2:
            return logits[:, 1] - logits[:, 0]
        raise ValueError("Expected binary classifier outputs.")

    def _classification_loss(self, logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        labels = self._soft_labels(labels).to(logits.device)
        pos_logit = self._positive_logit(logits)
        return F.binary_cross_entropy_with_logits(pos_logit, labels)

    def training_step(self, batch, batch_idx):
        x, y = batch
        preds = self.model(x)
        loss = self._classification_loss(preds, y)
        self.log("train_loss", loss, on_step=False, on_epoch=True, prog_bar=False)
        return loss
