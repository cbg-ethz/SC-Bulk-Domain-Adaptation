...
class SSDA4DrugLightningModule(pl.LightningModule):
    ...
    *working* Fast Gradient Method (FGM) implemented at the **feature level**.
    ...
    lambda_ent : float
        Single weight for the minimax entropy game on unlabeled target
        (mirrors the single `lamda` in the original utils.adentropy).
    lambda_fgm : float
    ...
    def training_step(self, batch: Dict[str, Tuple[torch.Tensor, Optional[torch.Tensor]]], batch_idx: int):
        ...
        if x_s is not None and y_s is not None:
            ...
            loss_s = self._classification_loss(logits_s, y_s)
            total_loss = total_loss + loss_s
            with torch.no_grad():
                acc_s = (logits_s.argmax(1) == self._harden_labels(y_s)).float().mean()
        ...
        if x_tl is not None and y_tl is not None and self.lambda_sup_t > 0:
            ...
            loss_tl = self._classification_loss(logits_tl, y_tl) * self.lambda_sup_t
            total_loss = total_loss + loss_tl
            with torch.no_grad():
                acc_tl = (logits_tl.argmax(1) == self._harden_labels(y_tl)).float().mean()
        ...
        if x_tu is not None:
            ...
            ent_pred_rev = self._entropy_from_logits(logits_pred_rev)
            ...
            ent_enc = self._entropy_from_logits(logits_enc)

            # Combine the two entropy terms under a single lambda (matching original API).
            # Multiply by 0.5 so the overall weight defaults to the original 0.1.
            ent_loss = (ent_pred_rev + ent_enc) * (0.5 * self.lambda_ent)
            total_loss = total_loss + ent_loss
            self.log("train/loss_pred_rev_ent", ent_pred_rev * (0.5 * self.lambda_ent), on_step=True, on_epoch=True, prog_bar=False)
            self.log("train/loss_enc_ent", ent_enc * (0.5 * self.lambda_ent), on_step=True, on_epoch=True, prog_bar=False)
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int):
        ...
        acc = (preds == self._harden_labels(y)).float().mean()
    ...
    @staticmethod
    def _soft_labels(labels: torch.Tensor) -> torch.Tensor:
        return labels.view(-1).float().clamp(0.0, 1.0)

    @staticmethod
    def _harden_labels(labels: torch.Tensor) -> torch.Tensor:
        return (labels.view(-1).float() >= 0.5).long()
    ...
    def _positive_logit(logits: torch.Tensor) -> torch.Tensor:
        ...
        raise ValueError("Expected binary classification logits.")

    def _classification_loss(self, logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        targets = self._soft_labels(labels).to(logits.device)
        pos_logit = self._positive_logit(logits)
        loss = F.binary_cross_entropy_with_logits(pos_logit, targets, reduction="none")
        if self._class_weights.numel() >= 2:
            weights = self._class_weights.to(logits.device)
            sample_weight = (1.0 - targets) * weights[0] + targets * weights[1]
